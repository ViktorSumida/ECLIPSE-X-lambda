...
if (graph == 1):
    ...
    transit_depth_phot_trans_lat_0 = np.genfromtxt("output_transit_depth(trans_lat=-40graus,f_spot=0.00).txt", delimiter=",")
    ...
    plt.text(950, 15500, '$Photosphere$', fontsize=17, bbox=dict(facecolor='white', alpha=0.5))
    ...
    epsilon_Rackham_trans_lat_20_fspot_8 = np.genfromtxt("output_epsilon_Rackham(trans_lat-46graus,f_spot=8%).txt", delimiter=",")

    wavelengths = np.genfromtxt("output_wavelengths.txt", delimiter=",")

    table_epsilon_ourWork = np.genfromtxt('output_epsilon_fixed_fspot_8% (our work).txt', delimiter=",",
                                          usecols=(0, 1, 2, 3, 4, 5), skip_header=1)
    table_epsilon_ourWork = np.transpose(table_epsilon_ourWork)
    ...
    graph5.plot(wavelengths, table_epsilon_ourWork[1], '-', linewidth=3, color=palette[1],
                label='$\epsilon$ - Trans. Lat. = 10$^{\circ}$')
    graph5.plot(wavelengths, table_epsilon_ourWork[2], '-', linewidth=3, color=palette[2],
                label='$\epsilon$ - Trans. Lat. = 20$^{\circ}$')
    graph5.plot(wavelengths, table_epsilon_ourWork[3], '-', linewidth=3, color=palette[3],
                label='$\epsilon$ - Trans. Lat. = 30$^{\circ}$')
    graph5.plot(wavelengths, table_epsilon_ourWork[4], '-', linewidth=3, color=palette[4],
