...
tabela_ExoCTK = xlrd.open_workbook('C:/Users/vikto/PycharmProjects/StarsAndExoplanets/'
                            'ExoCTK_results.xlsx')
aba_tabela = tabela_ExoCTK.sheet_by_name('Sheet1')
linhas_tabela = aba_tabela.nrows

coeficiente1 = np.zeros(linhas_tabela)
coeficiente2 = np.zeros(linhas_tabela)

####### Armazenando valores nos vetores das planilhas dentro do arquivo .xlsx #####
for i in range(linhas_tabela):
    coeficiente1[i] = aba_tabela.cell(i, 8).value
for i in range(linhas_tabela):
    coeficiente2[1] = aba_tabela.cell(i, 10).value

########################################################################################
########################################################################################

############ Estrutura de while para plotar uma estrela para cada ######################
############               comprimento de onda                    ######################
cont=0
while (cont<linhas_tabela):
    coeficienteHum = coeficiente1[cont]
    coeficienteDois = coeficiente2[cont]
        ...
        if x == 1:
            ...
            while aux == True:
                try:
                    ...
                    dec = ValidarEscolha(
                        "Deseja calular o semieixo Orbital do planeta através da 3a LEI DE KEPLER? 1. Sim 2.Não |")
                    if dec == 1:
...
while error == -1:
    try:
        escolha = ValidarEscolha(
            '\033[1;35mDeseja adicionar manchas em sua estrela? 1. Sim 2. Não |\033[m')  # x define a escolha, se haverá mancha ou não.
        if escolha == 1:
            ...
            while count != quantidade:  # o laço ira rodar a quantidade de manchas selecionada pelo usuario
                try:
                    ...
                    intensidadeMancha = Validar(
                        'Digite a intensidade da mancha em função da intensidade máxima da estrela:')
                    ...
                    estrela = estrela_.manchas(r, intensidadeMancha, lat,
                                               longt)  # recebe a escolha de se irá receber manchas ou não
...
while error == -1:
    try:
        escolha = ValidarEscolha(
            '\033[1;35mDeseja adicionar luas neste Planeta? 1. Sim 2. Não |\033[m')  # x define a escolha, se haverá luas ou não.
        if escolha == 1:
            lua = True
            ...
            while count != quantidade:  # o laço ira rodar a quantidade de luas selecionada pelo usuario
                try:
                    ...
                    massPlaneta = Validar(
                        'Digite a massa do Planeta (em unidades de massa de Júpiter ): ')  # deve ser convertido para Kg
                    ...
                    distancia = ((((perLua * 24. * 3600. / 2. / np.pi) ** 2) * G * (massPlaneta + mass)) ** (
                                1. / 3)) / raioStar
                    ...
                    # instanciando LUA
                    moon = eclipse.criarLua(rmoon, mass, raioPlanetaPixel, raioStar, tempoHoras, anguloInclinacao,
                                            periodo, distancia)
                    estrela = estrela_.getEstrela()
                    count += 1
                except Exception as erro:
